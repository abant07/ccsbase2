...
def build_full_fingerprint_vocabulary(database_file):
    db = Database(database_file)
    unique_smiles = db.read_df("SELECT DISTINCT smi FROM master_clean")["smi"]
    fp_dicts = calculate_sparse_fingerprints(unique_smiles)
    return build_fingerprint_vocabulary(fp_dicts, min_molecule_count=1)


def load_or_build_fingerprint_vocabulary(database_file, fp_vocab_file):
    if os.path.exists(fp_vocab_file):
        return joblib.load(fp_vocab_file)

    fp_vocab = build_full_fingerprint_vocabulary(database_file)
    joblib.dump(fp_vocab, fp_vocab_file)
    return fp_vocab
...
def vectorize_fingerprints_sparse(fp_dicts, fp_index) -> sp.csr_matrix:
    indptr = [0]
    indices = []
    data = []
    for fp in fp_dicts:
        for env_id, count in fp.items():
            column = fp_index.get(env_id)
            if column is not None:
    ...
    return sp.csr_matrix((data, indices, indptr), shape=(len(fp_dicts), len(fp_index)), dtype=np.float32)


def build_feature_matrix_sparse(base_features, fp_dicts, fp_index) -> sp.csr_matrix:
    return sp.hstack([
        np.array(base_features, dtype=np.float32),
        vectorize_fingerprints_sparse(fp_dicts, fp_index),
    ], format="csr")
