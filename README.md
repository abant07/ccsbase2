...
The full dataset that we have used for our models has been prebuilt and uploaded to the repository. To run the models, you will just need to convert the csv file into SQLite3 database using the schema below.

```ini
CREATE TABLE IF NOT EXISTS master_clean(" \
...
Alternatively, you can build the dataset from scratch. Run the code below in a separate file. Please note that this process takes a very long time as thousands of API calls are being sent to Pubchem to identify SMILES. 
...
ccsml = CCSDataIntegration("CCSMLDB.db")
ccsml.add_acs()
ccsml.add_ccsbase()
ccsml.add_allccs()
ccsml.add_metlin()
...
```python
classifier = ChemicalClassifier("CCSMLDB.db", min_class_count=30, seed=26)
classifier.fit()
classifier.predict()
```
...
conn = sqlite3.connect("CCSMLDB.db")
cur = conn.cursor()
...
conformers = Utils().calculate_conformers("CCSMLDB.db", smiles_reshaped)
```

## Training Model

To train the model, make sure you have ``CCSMLDB.db`` in the same directory as ``train.py``. This process takes 10 minutes to train fulldataset on a M4 Pro Macbook Pro.
